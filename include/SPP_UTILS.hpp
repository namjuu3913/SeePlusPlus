 ...
 *
 * @return uint8_t*   The beginning pointer of the frame data (when it fails, it
 * returns nullptr)
 *
 * @note
 *  - It uses aligned_frame_size to safely jumps over padding area when
 * accessing it
 * */
 ...
 *
 * @return uint8_t*   the beginning pointer of the frame in that time (returns
 * nullptr when it fails)
 * */
inline uint8_t *get_frame_at_time(const SPP_STRUCTS::VideoData *vd,
                                  double seconds) 
{
 ...
 *
 * @warning  It will free the internal data that allocated with
 * std::aligned_alloc first with std:free(), and then delete the struct
 * */
